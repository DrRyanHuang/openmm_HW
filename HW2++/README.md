...
本次作业用 YOLOX-Tiny 来训练COCO数据集

配置文件在:
```
yolox_tiny_8x8_300e_coco_mine.py
```
...
module load gcc/7.3
module load nccl/2.11.4-1 cuda11.1  # 注意不加这个会存在多卡报错的情况
...
# ---------- 单机单卡 ----------
python   tools/train.py   configs/yolox/yolox_tiny_8x8_300e_coco_mine.py   --auto-resume
# sbatch --gpus 1 run.sh

# ---------- 单机多卡 ----------
# ./tools/dist_train.sh  configs/yolox/yolox_tiny_8x8_300e_coco_mine.py   4  --auto-resume
...
# export PARTITION=dev   # 分区
# export JOB_NAME=YOLOX  # 任务名字

 
# GPUS=2 sh ./tools/slurm_train.sh ${PARTITION} ${JOB_NAME}  \
#     configs/yolox/yolox_tiny_8x8_300e_coco_mine.py \
#     tmp_work_dir_1 \
...
# sbatch -N 2 --gres=gpu:2 --qos=gpugpu run.sh
```

训练 log和训练模型文件在:

https://github.com/DrRyanHuang/mmdetection/releases/tag/HW2++
