...
# test模型
# python tools/test.py    configs/mask_rcnn/mask_rcnn_r50_fpn_1x_coco.py   work_dirs/mask_rcnn_r50_fpn_1x_coco/latest.pth   --eval bbox segm  --show-dir   work_dirs/mask_rcnn_r50_fpn_1x_coco  --show-score-thr 0.5

./tools/dist_train.sh  configs/mask_rcnn/mask_rcnn_r50_fpn_1x_coco.py   8 --auto-resume

# sbatch --gpus 1 run.sh
```

训练log, 模型pth, 和输出视频在 release 中:
https://github.com/DrRyanHuang/mmdetection/releases/tag/HW2

输出推理视频的 Python 启动命令:

```shell
python video_frame_demo.py test_video.mp4 \
       mask_rcnn_r50_fpn_1x_coco.py \
       epoch_199.pth \
